...
[data-testid="stSidebar"] { background: #1a2e1a; color: #e8f0e8; }
[data-testid="stSidebar"] label { color: #c8dfc8 !important; font-weight: 500; }
...
> *All data is based on real personal experience — 9 places visited over multiple years of camping across the country.*""")
    ...
    with qc1:
        st.markdown("""<div style="background:#eaf3ea;border-radius:8px;padding:12px;text-align:center;">🌱<br><b>Best for Beginners</b><br><small>Xızı · Easy · 106km</small></div>""", unsafe_allow_html=True)
    with qc2:
        st.markdown("""<div style="background:#e8f0ff;border-radius:8px;padding:12px;text-align:center;">❄️<br><b>Best Winter Camp</b><br><small>Zirəvun · Cabin · 172km</small></div>""", unsafe_allow_html=True)
    with qc3:
        st.markdown("""<div style="background:#fff3e0;border-radius:8px;padding:12px;text-align:center;">☁️<br><b>Most Magical</b><br><small>Hamosham · Above clouds · 304km</small></div>""", unsafe_allow_html=True)
    with qc4:
        st.markdown("""<div style="background:#fce4ec;border-radius:8px;padding:12px;text-align:center;">🏆<br><b>Highest Rated</b><br><small>Yeddiler · 5.0⭐ · 181km</small></div>""", unsafe_allow_html=True)
    with qc5:
        st.markdown("""<div style="background:#f3e5f5;border-radius:8px;padding:12px;text-align:center;">🚗<br><b>Closest to Baku</b><br><small>Xızı · 106km · Easy</small></div>""", unsafe_allow_html=True)
    st.markdown("---")
    ...
    if filtered.empty:
        ...
        for _, row in filtered.iterrows():
            ...
            card_html = f"""<div style="background:#ffffff;border-left:4px solid #3d6b3d;border-radius:8px;padding:18px 20px;margin-bottom:8px;box-shadow:0 2px 8px rgba(0,0,0,0.07);">
            ...
            if weather:
                st.markdown(f'<div style="background:#f0f7ff;border-radius:6px;padding:6px 12px;margin-top:-6px;margin-bottom:6px;font-size:0.82rem;color:#444;">🌡️ <b>{weather["temp"]}°C</b> &nbsp;|&nbsp; 💨 {weather["wind"]} km/h &nbsp;|&nbsp; 🌧️ {weather["precip"]}% rain chance today</div>', unsafe_allow_html=True)
...
# PAGE 2
    ...
    for _, row in filtered.iterrows():
        ...
        folium.Marker(location=[row["lat"], row["lon"]], popup=folium.Popup(popup_html, max_width=220), tooltip=row["name"], icon=folium.Icon(color=color, icon="leaf", prefix="fa")).add_to(m)
...
# PAGE 3
    ...
    with col1:
        st.markdown(f'<div style="background:#f4f8f4;border-radius:10px;padding:20px;text-align:center;border:1px solid #d0e4d0;"><div style="font-family:Georgia,serif;font-size:2rem;color:#3d6b3d;">{len(df)}</div><div style="font-size:0.8rem;color:#666;">Places Visited</div></div>', unsafe_allow_html=True)
    with col2:
        st.markdown(f'<div style="background:#f4f8f4;border-radius:10px;padding:20px;text-align:center;border:1px solid #d0e4d0;"><div style="font-family:Georgia,serif;font-size:2rem;color:#3d6b3d;">{df["region"].nunique()}</div><div style="font-size:0.8rem;color:#666;">Regions Explored</div></div>', unsafe_allow_html=True)
    with col3:
        st.markdown(f'<div style="background:#f4f8f4;border-radius:10px;padding:20px;text-align:center;border:1px solid #d0e4d0;"><div style="font-family:Georgia,serif;font-size:2rem;color:#3d6b3d;">{df["rating"].mean():.1f}⭐</div><div style="font-size:0.8rem;color:#666;">Average Rating</div></div>', unsafe_allow_html=True)
    with col4:
        st.markdown(f'<div style="background:#f4f8f4;border-radius:10px;padding:20px;text-align:center;border:1px solid #d0e4d0;"><div style="font-family:Georgia,serif;font-size:2rem;color:#3d6b3d;">70 ₼</div><div style="font-size:0.8rem;color:#666;">Price per Trip</div></div>', unsafe_allow_html=True)
    ...
    with col_a:
        ...
        fig1 = px.bar(region_rating, x="rating", y="region", orientation="h", color="rating", color_continuous_scale=["#c8dfc8","#3d6b3d"], labels={"rating":"Avg Rating","region":""})
        fig1.update_layout(plot_bgcolor="white", paper_bgcolor="white", coloraxis_showscale=False, height=360, margin=dict(l=10,r=10,t=10,b=10), xaxis=dict(range=[0,5.5]))
        st.plotly_chart(fig1, use_container_width=True)
    with col_b:
        st.markdown("### Camp Type Distribution")
        type_counts = df["type"].value_counts().reset_index()
        type_counts.columns = ["type","count"]
        fig2 = px.pie(type_counts, values="count", names="type", color_discrete_sequence=["#3d6b3d","#7aad7a","#c8dfc8"], hole=0.45)
        fig2.update_layout(plot_bgcolor="white", paper_bgcolor="white", height=360, margin=dict(l=10,r=10,t=10,b=10), legend=dict(orientation="h",yanchor="bottom",y=-0.2))
    ...
    fig3 = px.bar(diff_avg, x="difficulty", y="rating", color="rating", color_continuous_scale=["#c8dfc8","#3d6b3d"], labels={"rating":"Avg Rating","difficulty":"Difficulty to Reach"}, text="rating")
    fig3.update_traces(texttemplate="%{text:.1f} ⭐", textposition="outside")
    fig3.update_layout(plot_bgcolor="white", paper_bgcolor="white", coloraxis_showscale=False, height=320, margin=dict(l=10,r=10,t=10,b=10), yaxis=dict(range=[0,6]))
