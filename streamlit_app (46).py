...
span[data-baseweb="tag"] {
    ...
    color: #2d3a18 !important;
}
span[data-baseweb="tag"] span {
    color: #2d3a18 !important;
}
span[data-baseweb="tag"] svg {
    fill: #2d3a18 !important;
}
...
def load_data():
    data = [
        {"name": "Yeddiler Mountain", "region": "Guba", "type": "Wild Camping", "lat": 41.36, "lon": 48.52, "price_azn": 95, "rating": 5.0, "season": ["Spring","Summer","Autumn","Winter"], "highlight": "Dense mountain forest, total wilderness", "transport": "Bus + GAZ-66", "difficulty": "Hard", "favorite": False, "mosquitoes": False, "distance_km": 181, "description": "Remote mountain area accessible only by Soviet off-road truck. Pure wilderness with no facilities — real adventure camping."},
        ...
        {"name": "Canyon to Babadağ", "region": "Ismayilli", "type": "Wild Camping", "lat": 40.79, "lon": 48.14, "price_azn": 75, "rating": 5.0, "season": ["Summer"], "highlight": "Dramatic canyon + epic thunderstorm", "transport": "Bus + transfer", "difficulty": "Medium", "favorite": False, "mosquitoes": False, "distance_km": 172, "description": "Camped in a canyon on the trail to Babadağ peak. Experienced an all-night thunderstorm — terrifying and unforgettable."},
        {"name": "Zirəvun Pass", "region": "Ismayilli", "type": "Cabin", "lat": 40.72, "lon": 48.01, "price_azn": 75, "rating": 4.0, "season": ["Winter"], "highlight": "Snow-covered mountain pass in winter", "transport": "Bus + GAZ-66", "difficulty": "Hard", "favorite": False, "mosquitoes": False, "distance_km": 172, "description": "High mountain pass on the İsmayıllı–Qəbələ road. Snow in winter, accessible only by GAZ-66 off-road truck."},
        {"name": "Hamosham Valley", "region": "Astara", "type": "Wild Camping", "lat": 38.63, "lon": 48.42, "price_azn": 85, "rating": 5.0, "season": ["Spring"], "highlight": "Camping ABOVE the clouds ☁️", "transport": "Bus + transfer", "difficulty": "Hard", "favorite": True, "mosquitoes": False, "distance_km": 304, "description": "Author's favourite place. A cliff-top valley in the subtropical south. In April, you camp literally above the clouds."},
        {"name": "Xızı Colourful Hills", "region": "Khizi", "type": "Wild Camping", "lat": 40.91, "lon": 49.07, "price_azn": 75, "rating": 3.0, "season": ["Autumn"], "highlight": "Multicoloured clay hills on the way", "transport": "Bus + transfer", "difficulty": "Easy", "favorite": False, "mosquitoes": False, "distance_km": 106, "description": "Forest camping near the Caspian. The journey features stunning multicoloured hills — almost like a lunar landscape."},
        {"name": "Daşkəsən River Valley", "region": "Dashkasan", "type": "Wild Camping", "lat": 40.52, "lon": 46.08, "price_azn": 90, "rating": 4.0, "season": ["Summer"], "highlight": "Beautiful mountain rivers", "transport": "Bus + transfer", "difficulty": "Medium", "favorite": False, "mosquitoes": False, "distance_km": 422, "description": "Camping along mountain rivers in the lesser-visited Daşkəsən district. Fresh water and lush green valleys."},
        {"name": "Xaçmaz Forest & Sea", "region": "Khachmaz", "type": "Wild Camping", "lat": 41.46, "lon": 48.80, "price_azn": 75, "rating": 4.0, "season": ["Summer"], "highlight": "Beautiful forest near the Caspian Sea", "transport": "Bus", "difficulty": "Easy", "favorite": False, "mosquitoes": True, "distance_km": 169, "description": "Northern coast of Azerbaijan. Lovely forest but prepare for serious mosquitoes in summer. Sea swimming possible."},
        {"name": "Kəpəz Mountain (Göygöl)", "region": "Ganja / Kepez", "type": "Wild Camping", "lat": 40.59, "lon": 46.35, "price_azn": 180, "rating": 5.0, "season": ["Spring"], "highlight": "4am summit hike at sunrise", "transport": "Bus + transfer", "difficulty": "Hard", "favorite": False, "mosquitoes": False, "distance_km": 367, "description": "Camp at the base of Kəpəz mountain near the famous Göygöl lake. Wake at 4am for the summit hike and watch the sunrise from the top."},
    ]
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("### Filters")
    priority = ["Guba", "Ismayilli"]
    all_regions = priority + sorted([r for r in df["region"].unique() if r not in priority])
...
<div style="display:flex;gap:12px;margin-bottom:8px;align-items:stretch;">
  <div style="flex:1;background:#f5f5f5;border-left:4px solid #4a5a2a;border-radius:6px;padding:14px 16px;font-size:0.9rem;color:#1a1a1a;display:flex;align-items:center;">🌍 Most camps are in <b style="margin-left:4px;">northern Azerbaijan</b>&nbsp;(Guba, Ismayilli, Khachmaz)</div>
  <div style="flex:1;background:#f5f5f5;border-left:4px solid #4a5a2a;border-radius:6px;padding:14px 16px;font-size:0.9rem;color:#1a1a1a;display:flex;align-items:center;">☀️ <b style="margin-left:4px;">Summer</b>&nbsp;is the most popular season for camping</div>
  <div style="flex:1;background:#f5f5f5;border-left:4px solid #4a5a2a;border-radius:6px;padding:14px 16px;font-size:0.9rem;color:#1a1a1a;display:flex;align-items:center;">🏔️ Hard-to-reach places average <b style="margin-left:4px;">4.8⭐</b>&nbsp;vs Easy places <b>3.5⭐</b></div>
</div>
""", unsafe_allow_html=True)
    ...
    _card = lambda bg, icon, title, sub: f'<div style="background:{bg};border-radius:8px;padding:12px;text-align:center;border:1px solid {CARD_BORDER};">{icon}<br><b style="color:#1a1a1a;">{title}</b><br><small style="color:{CARD_META};">{sub}</small></div>'
    with qc1:
        st.markdown(_card("#f5f5f5","🌱","Best for Beginners","Xızı · Easy · 106km"), unsafe_allow_html=True)
    with qc2:
        st.markdown(_card("#f5f5f5","❄️","Best Winter Camp","Zirəvun · Cabin · 172km"), unsafe_allow_html=True)
    with qc3:
        st.markdown(_card("#f5f5f5","☁️","Most Magical","Hamosham · Above clouds · 304km"), unsafe_allow_html=True)
    with qc4:
        st.markdown(_card("#f5f5f5","🏆","Highest Rated","Yeddiler · 5.0⭐ · 181km"), unsafe_allow_html=True)
    with qc5:
        st.markdown(_card("#f5f5f5","🚗","Closest to Baku","Xızı · 106km · Easy"), unsafe_allow_html=True)
...
<div style="font-family:Georgia,serif;color:#1a1a1a;font-size:1.1rem;font-weight:bold;margin-bottom:6px;">{row['name']} {fav_badge}</div>
...
<p style="margin:8px 0 4px 0;color:#1a1a1a;font-size:0.9rem">{row['description']}</p>
